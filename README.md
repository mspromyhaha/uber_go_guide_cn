...
> my: 接口类型底层数据存的是指针,所以几乎不需要接口指针.

如果希望接口方法修改基础数据，则必须使用指针传递。

> my: 如果希望带接口参数的方法修改(接口)底层数据,则必须使用指针传递.
> my: 问题 - 接口类型的底层数据是私有的,不可以直接访问,怎么修改?
> my: 答 - 可以通过反射访问.
...
当通过指针访结构体时(可能并发修改数据,需要互斥), mutex 可以非指针形式作为结构体的字段(形如:struct{mu Mutex},通用的形式). 

对于非导出的结构体类型,想要使用 mutex 保护数据字段,可以嵌入 mutex 类型(形如:struct{Mutex},更宽松的形式).
...
包内`客观`上是无法阻止并发访问 struct 的字段的,因为即使带上 mutex 包内任意一段代码都可以`主观故意`旁路 mutex 的 Lock()/Unlock() 并发访问 struct 的字段,形如:
...
包内`主观`上是可以阻止并发访问 struct 的字段的,因为带上 mutex 后包内的代码可以`主观特意`通过 mutex 的 Lock()/Unlock() 互斥访问 struct 字段. `主观特意`加锁:
```
Lock()
t.feild = xxx
Unlock()
```
包内`主观特意加锁`后,只要锁机制不被破坏,就能达到互斥访问的目的.  
> 锁机制是: 
> - 先 Lock() 后 Unlock(),
> - 谁 Lock() 谁 Unlock().
...
因为包外的用户`主观`上可能是恶意的:
- 如果 Lock()/Unlock() 是可以包外访问的,那么锁机制就有可能被破坏.比如恶意用户包外调用 Lock() 后不调用 Unlock().
- 如果 Lock()/Unlock() 是包外不可访问的,那么就无法从包外通过他们来破坏锁机制.给他们带上非导出前缀就能私有化他们,如 x.Lock()/x.Unlock().
```
x.Lock()
t.feild = xxx
x.Unlock()
...
- 如果数据类型是非导出的--包级私有,比如 `type smap struct {...}`,那么类型嵌入式`type smap struct {Mutex ... }` 即可私有化锁方法.因为私有化类型一般定义私有化数据,如 `m *smap`, 此时 m.Lock()/m.Unlock() 已经是私有化的了.而如果使用类型组合式`type smap struct {mu Mutex ... }` 也可以私有化锁方法,如 `m *smap`, 此时 m.mu.Lock()/m.mu.Unlock() 私有化的两次.总的说类型组合式比类型嵌入式约束更紧,更通用.

- 如果数据类型是导出的--公有的,比如 `type Smap struct {...}`,那么类型嵌入式`type Smap struct {mu Mutex ... }` 才可以私有化锁方法.此时最坏情况是在包外用公有类型定义数据,如 `m *Smap`, 这时m.mu.Lock()/m.mu.Unlock() 还是私有化的了,m包外,mu还是私有的.而如果使用类型嵌入式`type smap struct {Mutex ... }` 就不可以私有化锁方法,如包外定义 `m *Smap`, 此时 m.Lock()/m.Unlock() 是公有的包外可以访问的,有可能破坏锁机制.可见类型组合式比类型嵌入式约束更紧,更通用.
