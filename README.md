...
***注解开始***
> my: 更好的翻译:  
> my: 当通过指针访结构体时(可能并发修改数据,需要互斥), mutex 可以非指针形式作为结构体的字段(形如:struct{mu Mutex},通用的形式). 
> my: 对于非导出的结构体类型,想要使用 mutex 保护数据字段,可以嵌入 mutex 类型(形如:struct{Mutex},更宽松的形式).
...
锁机制是先 Lock() 后 Unlock(),谁 Lock() 谁 Unlock().

现在问题转移到锁上来了:
包内`主观特意`不破环锁机制.
包外要`客观`不破坏锁机制,因为包外的用户`主观`上可能是恶意的.如果 Lock()/Unlock() 是可以包外访问的,那么锁机制就有可能被破坏.比如恶意用户包外调用 Lock() 后不调用 Unlock().如果 Lock()/Unlock() 是包外不可访问的,那么就无法从包外通过他们来破坏锁机制.给他们带上非导出前缀就能私有化他们,如 x.Lock()/x.Unlock().
```
x.Lock()
t.feild = xxx
x.Unlock()
